...
def get_item_and_quantity():
    ...
    Returns:
        Item name (string) and the quantity (integer).
    """
...
def calculate_price(item, quantity):
    ...
    Returns:
        float: The total price for the given item and quantity.
    """
    if item == "burger":
...
def main():
    """Calculates the subtotal for an order based on user input.
    Args:
        None
    Returns:
        None
